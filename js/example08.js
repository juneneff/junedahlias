$(function() {												//When the DOM is ready
	...
	$.ajax({												//Setup request
		beforeSend: function(xhr){							//Before sending data
	...
	function loadTimetable() {								//Declare function
		$.getJSON('data/example08.json')					//Try to collect JSON data
		.done( function(data){								//If successful
			times = data;									//Store it in a variable
		}).fail( function() {								//If a problem:; show message
			$('#event').html('Sorry! We can not load the timetable at the moment');
		});
	}
...
$('#content').on('click', '#event a', function(e) { 		//User cicks on a place
